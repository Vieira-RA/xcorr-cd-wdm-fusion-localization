"""
Load sweep_rotvec.npz and produce all figures, including control‑group comparison.
"""
import matplotlib
matplotlib.use("Agg")
...
# ========================= Load data ============================
data = np.load("sweep_rotvec.npz", allow_pickle=True)
...
ctrl_phase = data['ctrl_phase']
ctrl_phat  = data['ctrl_phat']
ctrl_int   = data['ctrl_int']
ctrl_par   = data['ctrl_par']
...
# Aggregated methods
metrics = {
    'Phase‑slope': compute_metrics(phase_est),
    'GCC‑PHAT':   compute_metrics(phat_est),
    'Integer peak': compute_metrics(int_est),
    'Parabolic peak': compute_metrics(par_est),
    'Control pair (phase)': compute_metrics(ctrl_phase),
    'Control pair (phat)':  compute_metrics(ctrl_phat),
    'Control pair (int)':   compute_metrics(ctrl_int),
    'Control pair (par)':   compute_metrics(ctrl_par),
}
...
ax.set_title('Rotation‑vector sensing principle')
ax.axis('off')
plt.tight_layout()
plt.savefig('fig01_rotvec_schematic.png', dpi=150)
plt.close()

# ========================= Figure 3: Std heatmaps ==============
for name, (std, bias, succ_rel, succ_abs) in metrics.items():
...
if len(demo_phase_pairs) > 0:
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig('fig06_rotvec_pairwise_scatter.png', dpi=150)
    plt.close()
else:
...
ax.set_title('Execution time vs bandwidth (rotation‑vector)')
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('fig07_rotvec_timing.png', dpi=150)
plt.close()

# ========================= Figure 8: Global pair distribution =====
# For rotation vector we don't have inlier arrays for control pair,
# only for aggregated methods. We'll just plot the four main methods.
...
plt.tight_layout()
plt.savefig('fig08_rotvec_global_pair_distribution.png', dpi=150)
plt.close()

print("All figures saved from sweep_rotvec.npz")