#!/usr/bin/env python3
"""
Multi‑count post‑processing script for CD localisation sweep (parallelised).
Uses joblib to parallelise over noise realisations.
...
def process_one_realisation(bw, f_max, dt, b_lp, a_lp, stokes_clean, phi_mag_clean,
                            ...
                            i_snr, snr_db, i_real):
    """
    Process a single (SNR, realisation) for a given bandwidth.
    Returns a dictionary with distance estimates for all methods.
    """
...
for channel_count in channel_counts:
    ...
    phase_est_2d = np.full(shape, np.nan);   phat_est_2d  = np.full(shape, np.nan)
    int_est_2d   = np.full(shape, np.nan);   par_est_2d   = np.full(shape, np.nan)
    phase_est_rv = np.full(shape, np.nan);   phat_est_rv  = np.full(shape, np.nan)
    int_est_rv   = np.full(shape, np.nan);   par_est_rv   = np.full(shape, np.nan)

    ctrl_2d = { 'phase': np.full(shape, np.nan), 'phat': np.full(shape, np.nan),
                'int': np.full(shape, np.nan),   'par': np.full(shape, np.nan) }
    ctrl_rv = { 'phase': np.full(shape, np.nan), 'phat': np.full(shape, np.nan),
                'int': np.full(shape, np.nan),   'par': np.full(shape, np.nan) }
    ...
    for i_bw, bw in enumerate(tqdm(BANDWIDTHS, desc=f"BW ({channel_count}ch)")):
        bw_start = time.time()
        jones_file = os.path.join(output_dir, f"jones_bw_{bw:.0f}Hz.npz")
        ...
        # ===== CORRECT SNR DEFINITIONS (FAIR for both signal types) =====
        # Stokes: use CLEAN ROTATED vectors for peak power
        ...
        # Rotation‑vector: use AC (DC‑free) peak power
        ...
        # Parallel loop over SNRs and realisations
        jobs = []
        ...
        # Unpack results into storage arrays
        for job_idx, (i_snr, snr_db) in enumerate([(i_snr, snr_db) for i_snr, snr_db in enumerate(SNRS) for _ in range(N_REAL)]):
            # job_idx maps to the flattened order: i_snr * N_REAL + i_real
            i_snr = job_idx // N_REAL
            i_real = job_idx % N_REAL
            res = results[job_idx]

            # 2D estimates
            phase_est_2d[i_bw, i_snr, i_real] = res['est_2d']['phase']
            phat_est_2d[i_bw, i_snr, i_real]  = res['est_2d']['phat']
            int_est_2d[i_bw, i_snr, i_real]   = res['est_2d']['int']
            par_est_2d[i_bw, i_snr, i_real]   = res['est_2d']['par']

            # RV estimates
            phase_est_rv[i_bw, i_snr, i_real] = res['est_rv']['phase']
            phat_est_rv[i_bw, i_snr, i_real]  = res['est_rv']['phat']
            int_est_rv[i_bw, i_snr, i_real]   = res['est_rv']['int']
            par_est_rv[i_bw, i_snr, i_real]   = res['est_rv']['par']

            # control pairs
            for k in ['phase','phat','int','par']:
                ctrl_2d[k][i_bw, i_snr, i_real] = res['ctrl_2d'][k]
                ctrl_rv[k][i_bw, i_snr, i_real] = res['ctrl_rv'][k]
    ...
    methods = {}
    for name, arr in [('Phase‑slope 2D', phase_est_2d), ('GCC‑PHAT 2D', phat_est_2d),
                      ('Integer peak 2D', int_est_2d), ('Parabolic peak 2D', par_est_2d),
                      ('Phase‑slope RV', phase_est_rv), ('GCC‑PHAT RV', phat_est_rv),
                      ('Integer peak RV', int_est_rv), ('Parabolic peak RV', par_est_rv)]:
        methods[name] = compute_metrics(arr)
    for prefix, ctrl_dict in [('Ctrl 2D', ctrl_2d), ('Ctrl RV', ctrl_rv)]:
